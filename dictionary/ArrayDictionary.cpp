...
bool ArrayDictionary<KeyType, ItemType>::add(const KeyType &searchKey, const ItemType &newItem) {
    ...
    if (ableToInsert) {
        ...
        while( (index > 0) && (searchKey < items[index - 1].getKey()) ){ /** Moving all items over by 1.*/
            items[index] == items[index - 1];
            index--;
        }
        items[index] = Entry<KeyType, ItemType>(searchKey, newItem); /** Now indexed at the place we want to insert */
        itemCount++;
    }
...
int ArrayDictionary<KeyType, ItemType>::findEntryIndex(int firstIndex, int lastIndex, const KeyType &searchKey) const {
    ...
    else
    {
        ...
        if(searchKey == items[midIndex].getKey())
            index = midIndex;
        else if(searchKey < items[midIndex].getKey())
            index = findEntryIndex(firstIndex, midIndex - 1, searchKey);
        else
            index = findEntryIndex(midIndex + 1, lastIndex, searchKey);
    }
