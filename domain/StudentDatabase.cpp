...
void StudentDatabase::displayStudents() {
    ...
    students.traverse(displayStudent);
...
void StudentDatabase::displayCourses() {
    ...
    //YOUR CODE HERE
    courses.traverse(displayCourse);
...
void StudentDatabase::displayRecords() {
     ...
     */
     vector<shared_ptr<Transcript>> transcriptVect = transcripts.toVector();
     for(int i = 0; i < transcripts.getCurrentSize(); i++){
         cout<<left <<
         setw(10) << transcriptVect[i]->getTranscriptId() << " -- " <<
         setw(10) << transcriptVect[i]->getStudentId() << " -- " <<
         setw(10) << transcriptVect[i]->getCourseId() << " -- " <<
         setw(15) << transcriptVect[i]->getGrade() << endl;
     }
...
void StudentDatabase::displayTranscript() {
    ...
    vector<shared_ptr<Transcript>> transcriptVect = transcripts.toVector();
    for(int i = 0; i < transcripts.getCurrentSize(); i++){
        shared_ptr<Student> tempStudent = students.getItem(transcriptVect[i]->getStudentId());
        shared_ptr<Course> tempCourse = courses.getItem(transcriptVect[i]->getCourseId());
        cout<<left <<
            setw(10) << transcriptVect[i]->getTranscriptId() << " -- " <<
            setw(10) << transcriptVect[i]->getStudentId() << " -- " << setw(20) << tempStudent->getName() << " -- " <<
            setw(10) << transcriptVect[i]->getCourseId() << " -- " << setw(30) << tempCourse->getDescription() << " -- " <<
            setw(15) << transcriptVect[i]->getGrade() << endl;
    }
